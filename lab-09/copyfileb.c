...
int main(int argc, char* argv[]){
	...
	while(1){
		...
		if (numRead <=0){
			...
			//printf("Time_spent = %fs\n",time_spent);
			printf("Copying from %s to %s with buffer size %lu bytes ... %d bytes copied successfully in %fs.\n",argv[1],argv[2],bufLen,file_size(argv[1]),time_spent);
			return 0;
		}
